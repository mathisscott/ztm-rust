...
struct Opt {
    ...
    #[structopt(default_value = "http://localhost:8000", env = "CLIPSTASH_ADDR")]
    addr: String,
...
fn get_clip(addr: &str, ask_svc: GetClip, api_key: ApiKey)
...
fn new_clip(addr: &str, ask_svc: NewClip, api_key: ApiKey)
    -> Result<Clip, Box<dyn Error>>
{
    let client = reqwest::blocking::Client::builder().build()?;
...
fn update_clip(addr: &str, ask_svc: UpdateClip, api_key: ApiKey)
    -> Result<Clip, Box<dyn Error>>
{
    let client = reqwest::blocking::Client::builder().build()?;
    let addr = format!("{}/api/clip", addr);
